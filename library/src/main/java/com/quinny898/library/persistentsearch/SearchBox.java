...
public class SearchBox extends RelativeLayout {
	...
	private View.OnClickListener micListener;
	...
	public SearchBox(final Context context, AttributeSet attrs, int defStyle) {
		...
		mic.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				} else if (micListener != null) {
					micListener.onClick(v);
				} else {
					micClick();
				}
			}
			...
			public void afterTextChanged(Editable s) {
				if (s.length() > 0) {
					...
					mic.setImageDrawable(getContext().getResources().getDrawable(
							R.drawable.ic_close_grey600_36dp));
					updateResults();
				} else {
		...
		mSearchFilter = new SearchBox.SearchFilter() {
			@Override
	...
	private void micStateChanged() {
		if(!TextUtils.isEmpty(getSearchText()) && micListener != null) {
	...
	public void setMicClickListener(View.OnClickListener listener) {
    ...
    public boolean dispatchKeyEvent(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.KEYCODE_BACK && materialMenu.getState() == IconState.ARROW){
